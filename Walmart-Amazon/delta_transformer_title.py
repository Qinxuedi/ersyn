...
transformer_data = TabularDataset('csv_files/seq2seq_title_jac_delta_{}.csv'.format(DELTA_ID),format='csv',skip_header=True,
                    fields=[('seq1',SEQ),('seq2',SEQ),('title_title_jac_qgm_3_qgm_3',SIM)])
transformer_train_data = transformer_data

SEQ.build_vocab(transformer_data.seq1, transformer_data.seq2)

vocab_path='models/title/delta_{}_transformer_vocab.txt'.format(DELTA_ID)
...
N_EPOCHS = 400
CLIP = 1
log_path='models/title/delta_{}_transformer_log.txt'.format(DELTA_ID)
...
for epoch in range(N_EPOCHS):
    ...
    torch.save(transformer_model.state_dict(), 'models/title/delta_{}_transformer_model_{}.pt'.format(DELTA_ID,epoch))
